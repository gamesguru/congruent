...
### Running console commands without a TTY

The admin console has three different entry modes:

- `--attach` opens the interactive REPL and still needs a real TTY because it uses readline.
- `--attach --execute "..."` runs one or more commands against a running server without opening the REPL.
- `--execute "..."` on the main server binary runs commands during startup instead.

If you only need a one-shot command in automation, prefer `--attach --execute` for a running server or `--execute` during startup.
