...
If you want to run admin commands against an already-running server without opening the interactive REPL, use `--attach --execute "..."` instead. In that mode the attach client connects to `console.sock`, sends each command line directly, and exits without requiring a TTY.
