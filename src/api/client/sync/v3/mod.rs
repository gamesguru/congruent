...
fn is_sync_response_empty(val: &serde_json::Value) -> bool {
	...
	// no summary) is NOT a real delta: returning it early forces the client into
	// a second round-trip to fetch the actual event, compounding latency (this is
	// the server-side half of the TestSpoofedEventSenderHandling 1s-window race).
	...
	if let Some(rooms) = obj.get("rooms").and_then(|r| r.as_object()) {
		...
		let invited_has_delta = rooms
			.get("invite")
			.is_some_and(|i| !i.as_object().is_none_or(|m| m.is_empty()));

		let knocked_has_delta = rooms
			.get("knock")
			.is_some_and(|k| !k.as_object().is_none_or(|m| m.is_empty()));
