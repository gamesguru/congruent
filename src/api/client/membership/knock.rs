...
fn knock_room_by_id_helper<'a>(
	services: &'a Services,
	sender_user: &'a UserId,
	room_id: &'a RoomId,
	reason: Option<String>,
	servers: &'a [OwnedServerName],
) -> Pin<Box<dyn Future<Output = Result<knock_room::v3::Response>> + Send + 'a>> {
	Box::pin(async move {
		...
		if let JoinRule::KnockRestricted(restricted) = &join_rule {
			...
			if user_meets_restrictions {
				...
				.await
				{
					...
					| Err(e) => {
						debug_warn!(
							"Failed to convert knock to join for {sender_user} in {room_id}: \
							 {e:?}"
						);
						...
						let local_knock = server_in_room
							|| servers.is_empty() || (servers.len() == 1
...
fn knock_room_helper_local<'a>(
	...
	Box::pin(async move {
		...
		let mut knock_event_stub =
			serde_json::from_str::<CanonicalJsonObject>(make_knock_response.event.get())
				.map_err(|e| {
...
fn knock_room_helper_remote<'a>(
	...
	Box::pin(async move {
		...
		let state = send_knock_response
			...
			.iter()
			.map(|event| {
				serde_json::from_str::<CanonicalJsonObject>(event.clone().into_json().get())
			})
		...
		for event in state {
			...
			let Ok(event_type) =
				serde_json::from_value::<StateEventType>(event_type.clone().into())
			else {
